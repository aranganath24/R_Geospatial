...
author: "New York University Data Services (Last Updated: 9/1/2020)"
...
setwd("~/Documents/NYU GIS Workshops/R Workshop/data")
```
...
```{r enter CSV into memory, results='false', message=FALSE}
...
```{r data viewer, results='false', message=FALSE}
View(trade_gdp)
```
...
We can find a useful shapefile of world country boundaries by searching the NYU SDR. In particular, see the following link:
<https://geo.nyu.edu/catalog/stanford-ps917hm2349> 
...
```{r plot shapefile, fig.asp=0.5}
tm_shape(world_shapefile)+
  tm_polygons()
```

The ```tm_shape``` function specifies the file we wish to map, while the ```tm_polygons()``` function tells tmap to draw the polygons that comprise this shapefile. Though you should get an intuitive sense of tmap's syntax as we go, we won't explicitly cover the tmap package's syntax in this tutorial; if you expect to use tmap in the future, and want a more formal guide, an excellent place to start is the package's official documentation, which is very thorough and user-friendly, and can be found [here](https://www.rdocumentation.org/packages/tmap/versions/2.3-1). 
...
```{r make orange polygons, fig.asp=0.5}
tm_shape(world_shapefile)+
  tm_polygons("orange")
```
...
```{r make cyan polygons, fig.asp=0.5}
tm_shape(world_shapefile)+
  tm_polygons("lightcyan2")
```

For a concise overview of color-related packages in R (which tmap draws on), see this useful [guide](https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf). 
...
```{r make cyan polygons and delete frame, fig.asp=0.5}
tm_shape(world_shapefile)+
  tm_polygons("lightcyan2")+
...
```{r view shapefile interactively after setting view mode}
tm_shape(world_shapefile)+
  tm_polygons("lightcyan2")
```
...
```{r join WB data frame to shapefile, warning=FALSE}
...
```{r view the shapefile created through the join above}
...
```{r confirm class of field we want to map}
...
```{r convert field you want to map to numeric, warning=FALSE}
...
```{r assign preliminary map to object}
...
To learn more about color and palette options, refer again to this [guide](https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf). 

To learn more about our options for classifying your data (aside from Jenks), see the [documentation](https://rdrr.io/cran/tmap/man/tm_polygons.html) for the tm_polygons function within tmap. We could also pull up this documentation within R Studio by simply typing the following (which will bring up the relevant documentation in R Studio's "Help" tab):
...
The workshop data you downloaded contains a shapefile of Malawi constituency borders from the NYU Spatial Data Repository. The shapefile can also be found [here](https://geo.nyu.edu/catalog/stanford-hs879xj7186). Bring the shapefile of Malawi constituencies into R Studio and assign it to an object called "malawi_shapefile":
...
Both of the books mentioned above include a discussion of raster analysis in R, which is beyond the scope of this tutorial. If you are especially interested in raster analysis, an additional resource worth consulting (in addition to the two books mentioned above), is a tutorial, written by Robert Hijmans, on [Spatial Data Science with R](https://rspatial.org/index.html). It provides a good overview of raster-specific packages. It also offers useful coverage of spatial statistics and modeling. 

# References

Brunsdon, Chris, and Lex Comber. 2019. *An Introduction to R for Spatial
   Analysis and Mapping (2nd ed.).* London: Sage Publications, Ltd. 
...
Hijmans, Robert J. "Spatial Data Science With R." Accessed August 28, 2020.
   <https://rspatial.org/> 
